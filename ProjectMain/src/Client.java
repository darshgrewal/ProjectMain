...
public class Client extends TransferHub{
	...
	private void printOutgoingPacketInfo(DatagramPacket packet) {
		...
		// used to keep the byte array of the packet with the correct size
	...
	private void handleReadReq(String serverReadFile, String clientWriteFile) {
		...
		try {
			...
			if (e1.getMessage().contains("Permission denied")) {
				System.out.println("Access Violation happened for RRQ on client-side.");
				return;
			}
			e1.printStackTrace();
			System.exit(1);
		...
		while(moreToCome) {
			...
			if ( (data[1] != 3) && (data[1] != 5) ) {
				System.out.println("Received packet from server is neither DATA nor ERR.");
				System.exit(1);
			}

			// Handle Error packets from server
			if (data[1] == 5) {
				String errorMssg = getErrorMessage(receivePacket.getData(), receivePacket.getLength());
	...
	private String getErrorMessage(byte[] receiveBytes, int length) {
		String output = "Received error message from ";
		if (isNormal)
			output += "server ";
		else
			output += "intermediate host ";

		output += "contains the error message: ";

		output += new String(receiveBytes,3,length - 4);
	...
	private void handleWriteReq(String clientReadFile, String serverWriteFile) {
		...
		try {
			...
			if (e1.getMessage().contains("Permission denied")) {
				System.out.println("Access Violation happened for WRQ on client-side.");
				return;
			}
			e1.printStackTrace();
			System.exit(1);
		...
		if (ack[0] != 0) {
			System.out.println("First byte of received packet from server is not 0.");
			System.exit(1);
		}
		if ( (ack[1] != 4) && (ack[1] != 5) ) {
			System.out.println("Received packet from server is neither ACK nor ERR.");
			System.exit(1);
		}
		...
		while(!isQuit) {
			...
			if (req.equalsIgnoreCase("W")) {
				...
				if (f.exists() && !f.isDirectory()) {
					...
					System.out.println("Requested file does not exist.");
					cAndSendError(sendReceiveSocket, "Requested file does not exist", 1,SERVER_PORT);
				}
